...
/**
 * Serverless platforms cap the request body — Vercel's limit is 4.5 MB — so an
 * upload larger than this never reaches the handler at all. Keeping the check
 * here means the director gets a clear message instead of a platform error.
 * Raise POCKET_STUDIO_MAX_AUDIO_MB where the deployment allows bigger bodies.
 */
const MAX_MB = Number.parseFloat(process.env.POCKET_STUDIO_MAX_AUDIO_MB ?? "4");
const MAX_BYTES = (Number.isFinite(MAX_MB) && MAX_MB > 0 ? MAX_MB : 4) * 1024 * 1024;
